...
var UI = (function () {
  function UI(images) {
    _classCallCheck(this, UI), this.init(images);
  }
  return (
    _createClass(UI, [
      {
        ...
        value: function (images) {
          ...
          (this.toggleText = ["Show menu", "Hide menu"]),
            ...
            this.engine.on("setImage", function (img) {
              _this.engine.set(
                "activeImage",
                _this.engine.get("images").indexOf(_this._getRelativeSrc(img))
              );
            }),
            this.engine.on("setImageButton", function (img) {
              _this.stopRotate(), _this.setImage(img);
            }),
            this.engine.on("toggle", function () {
              _this.engine.set("show", !_this.engine.get("show")),
                _this.engine.set(
                  "toggleText",
                  _this.toggleText[~~_this.engine.get("show")]
                );
            }),
            this.engine.on("togglePlay", function () {
              if (
                ...
                _this.engine.set(
                  "playText",
                  _this.playPause[~~!_this.engine.get("rotating")]
                ),
                _this.engine.get("rotating"))
              )
                _this.stopRotate();
              else {
                _this.engine.set("rotating", !0);
      ...
      },
      {
        ...
        value: function (img) {
          var path = void 0;
          return (
            (path = img.node ? img.node.src : img),
      ...
      },
      {
        ...
        value: function (index) {
          ...
          -1 === index && (index = imageCount - 1);
          var newIndex = index % imageCount,
            newImage = this.engine.get("images")[newIndex];
          this.engine.set("activeImage", newIndex), this.setImage(newImage);
        },
      },
      {
        key: "startRotate",
