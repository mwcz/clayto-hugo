...
function handlePopupsDisabled() {
  ...
  if (disabled) {
    document
      .querySelector(".popup-showdown")
      .setAttribute("data-popups", "disabled");
  } else {
    document.querySelector(".popup-showdown").removeAttribute("data-popups");
  }
  return disabled;
}
...
function start() {
  ...
  document
    .querySelector(".popup-showdown")
    .setAttribute("data-state", "playing");
  playing = true;
...
function stop(early) {
  document
    .querySelector(".popup-showdown")
    .setAttribute("data-state", "gameover");
  playing = false;
...
function popup(x, y) {
  ...
  pop.addEventListener("beforeunload", function(e) {
    handlePopupClosed(e, id);
  });

  pop.addEventListener("load", function() {
    // pop.document.querySelector('.popup-showdown').querySelector('h1').innerText = `POPUP`;
    pop.document
      .querySelector(".popup-showdown")
      .querySelector("h2").innerText = id.toUpperCase();
  });
...
function updateInterval() {
  pop_interval = Math.max(
    POP_INTERVAL_MIN,
    POP_INTERVAL_INITIAL * (1 - smoothstep(timeprogress()))
  );
}
...
function updateSize() {
  pop_width =
    PAGE_WIDTH *
    ((POP_MAX_WIDTH - POP_MIN_WIDTH) *
      (1 - smoothstep(timeprogress()) + POP_MIN_WIDTH));
  pop_height =
    PAGE_HEIGHT *
    ((POP_MAX_HEIGHT - POP_MIN_HEIGHT) *
      (1 - smoothstep(timeprogress()) + POP_MIN_HEIGHT));
}
...
function showWinScreen(win, early) {
  if (win && !early) {
    document.querySelector(".popup-showdown").setAttribute("data-won", "");
  } else {
    document.querySelector(".popup-showdown").setAttribute("data-lost", "");
  }
  if (early) {
    // show crazy number if the player quit early
    document.querySelector("#final-close-count").textContent = -Math.floor(
      Math.random() * 1e6
    );
  } else {
